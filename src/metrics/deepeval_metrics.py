...
class SceneBoundaryMetric(BaseMetric):
    ...
    def _evaluate_structure(self, json_data: List[Dict]) -> float:
        ...
        # 2. 场景设置完整性
        settings_score = sum(
            1 for scene in json_data 
            if self._is_valid_setting(scene.get("setting", ""))
        ) / len(json_data)
        ...
        field_score = sum(
            all(field in scene for field in required_fields)
...
"""
    ...
    def _is_valid_setting(self, setting: str) -> bool:
        ...
        # 检查是否包含时间
        has_time = any(x in setting for x in ['日', '夜', '晨', '昏', 'DAY', 'NIGHT'])
...
class CharacterExtractionMetric(BaseMetric):
    ...
    def _evaluate_consistency(
        ...
        scene_characters: Dict[str, List[int]]
    ) -> float:
        """评估角色一致性"""
    ...
    def _evaluate_importance(
        ...
        # 记录主要角色
        self.details["main_characters"] = [
            char for char, count in char_frequency.most_common(3)
        ]
...
"""
    ...
    def _generate_character_reason(self):
        ...
        if "main_characters" in self.details:
            reasons.append(
                f"识别的主要角色: {', '.join(self.details['main_characters'])}"
            )
...
class SelfConsistencyMetric(BaseMetric):
    ...
    def measure(self, test_case: LLMTestCase) -> float:
        ...
        # 加权平均
        self.score = sum(
            self.field_scores.get(field, 0) * weight
...
if __name__ == "__main__":
    ...
    results = evaluate(
        ...
        metrics=[boundary_metric, character_metric]
    )
